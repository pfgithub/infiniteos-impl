...
const MainMenuButton = ({ children, onClick, id }: { children: React.ReactNode, onClick: () => void, id: string }) => (
  <button
    id={id}
    className="w-full max-w-xs px-6 py-3 text-xl font-['MedievalSharp'] text-yellow-100 bg-gray-800/70 border-2 border-yellow-400/50 rounded-md shadow-[0_0_15px_rgba(250,204,21,0.4)] hover:bg-gray-700/70 hover:text-white hover:border-yellow-300 transition-all duration-300 ease-in-out transform hover:scale-105"
    onClick={onClick}
  >
    {children}
  </button>
...
const ArcaneLegacyMainMenu: React.FC<ArcaneLegacyMainMenuProps> = ({ id }) => {
  ...
  const menuContent = (
    <div className="bg-black/60 backdrop-blur-sm p-8 rounded-lg shadow-2xl flex flex-col items-center">
      <h1 className="text-6xl font-extrabold text-yellow-200 mb-2 font-['MedievalSharp']" style={{ textShadow: '2px 2px 8px #ca8a04' }}>
        Arcane Legacy
      </h1>
      <p className="text-yellow-100 mb-10 text-lg font-['MedievalSharp']">An adventure awaits.</p>
